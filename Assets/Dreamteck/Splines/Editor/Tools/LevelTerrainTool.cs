...
{
    public class LevelTerrainTool : ISplineTool
    {
        ...
        {
            bool getComputers = computers.Count == 0;
            for (int i = 0; i < Selection.gameObjects.Length; i++)
            {
                if (terrain == null)  terrain = Selection.gameObjects[i].GetComponent<Terrain>();
                if (!getComputers) continue;
                SplineComputer comp = Selection.gameObjects[i].GetComponent<SplineComputer>();
                if (comp != null) computers.Add(comp);
            }
        ...
        public void Draw(Rect windowRect)
        {
            ...
            int comCount = Mathf.Max(computers.Count, 2);
            GUI.Box(new Rect(windowRect.x+5, 5, windowRect.width-5, 40 + comCount * 20), "");
            GUI.color = Color.white;

            EditorGUILayout.Space();

            GUILayout.BeginHorizontal(GUILayout.MaxHeight(40+comCount*20), GUILayout.MaxWidth(windowRect.width-15));
            ...
            GUILayout.BeginVertical();
                GUILayout.Label("Spline Computer:");
                for(int i = 0; i < computers.Count; i++) {
                    GUILayout.BeginHorizontal();
                    GUILayout.Label(computers[i].name);
                ...
                if (!found) computers.Add(newComp);
