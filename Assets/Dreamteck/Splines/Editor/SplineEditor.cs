...
namespace Dreamteck.Splines
{
    ...
    public class SplineEditor : Editor 
    {
        ...
        private static void NewSplineNode()
        {
            ...
            {
                if(EditorUtility.DisplayDialog("Make child?", "Do you want to make the new node a child of the selected " + Selection.activeGameObject + "?", "Yes", "No"))
                {
        ...
        void OnEnable()
        {
            ...
            if (EditorPrefs.HasKey("SplineEditor_appendMode")) appendMode = EditorPrefs.GetInt("SplineEditor_appendMode");
            ...
            SceneView.onSceneGUIDelegate += PostScene;
        }
        ...
        private static void AlwaysDrawPaths(SceneView current)
        {
            for (int i = 0; i < drawComputers.Count; i++)
            {
                ...
                DrawSplineComputer(drawComputers[i], SceneView.currentDrawingSceneView.camera, false, false);
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (computer.pointCount < 2 || computer.hasMorph) GUI.color = new Color(1f, 1f, 1f, 0.5f);
            else GUI.color = Color.white;
            if (GUILayout.Button("Inverse Point Order"))
            {
                InversePointOrder();
            }
        ...
        {
            points = computer.GetPoints();
            for(int i = 0; i < Mathf.Floor(points.Length/2); i++)
            {
        ...
        public void SelectPoint(int index)
        {
            ...
            rotationEditor.Reset(ref points, ref selectedPoints);
            scaleEditor.Reset(ref points, ref selectedPoints);
            Repaint();
        }
        ...
        void OnSceneGUI()
        {
            ...
            if (!editorCamera.pixelRect.Contains(Event.current.mousePosition))
            {
                ...
                control = controlDown = controlUp = altDown = altUp = alt = mouseLeftDown = mouseRightDown = mouseLeftUp = mouserightUp = mouseLeft = mouseRight = false;
            ...
            if (tool != PointTool.Create || createPointMode == 1 || toolbar.mouseHovers)
            {
                DrawComputer(computer);
                for (int i = 0; i < computers.Count; i++)
                {
                    DrawSplineComputer(computers[i], SceneView.currentDrawingSceneView.camera, showThickness, thicknessAutoRotate, 0.5f);
                }
            } else
            {
                if (createPointVisualizer != null)
                {
                    ...
                    DrawSpline(createPointVisualizer, computer.editorPathColor, 0.0, 1.0, showThickness, thicknessAutoRotate);
                }
            }
            ...
            if (Tools.current == Tool.None && (!computer.hasMorph || MorphWindow.editShapeMode)) EditPoints();
            ...
            if (Tools.current == Tool.None)
            {
                ...
                computer.transform.position = Handles.FreeMoveHandle(computer.transform.position, Quaternion.identity, axisSize * 0.07f, Vector3.zero, Handles.RectangleCap);
            ...
            Handles.BeginGUI();
            if (!defaultEditor.isDragging && Tools.current == Tool.None) toolbar.Draw();
            ...
            if (GUI.changed || rebuild)
            {
                ...
                if (!GUI.changed) points = computer.GetPoints();
                UpdateComputer();
                EditorUtility.SetDirty(computer);
            }

            if (emptyClick && !alt && !control)
            {
        ...
        private void CreateSplinePoint(Vector3 position, Vector3 normal)
        {
            ...
            {
                GameObject obj = new GameObject("Node_P" + (points.Length - 1));
                ...
                if (appendMode == 0)
                {
                    ...
                    node.AddConnection(computer, points.Length - 1);
                }
                else
                {
                    node.transform.position = points[0].position;
                    node.AddConnection(computer, 0);
                }
            }
        ...
        public static void DrawSplineComputer(SplineComputer comp, Camera editorCamera, bool showThickness, bool thicknessAutoRotate, float alpha = 1f, double fromPercent = 0.0, double toPercent = 1.0, bool autoColor = true)
        {
            if (comp == null) return;
            ...
            if (!showThickness)
            {
                ...
                Vector3 fromPoint = comp.EvaluatePosition(percent);
                while (true)
                {
                    percent = DMath.Move(percent, toPercent, add);
                    Vector3 toPoint = comp.EvaluatePosition(percent);
            ...
            } else
            {
                ...
                if (thicknessAutoRotate) fromNormal = (editorCamera.transform.position - from.position).normalized;
                Vector3 fromRight = Vector3.Cross(from.direction, fromNormal).normalized * from.size * 0.5f;
                while (true)
                {
                    percent = DMath.Move(percent, toPercent, add);
                    SplineResult to = comp.Evaluate(percent);
                    Vector3 toNormal = to.normal;
                    if (thicknessAutoRotate) toNormal = (editorCamera.transform.position - to.position).normalized;
        ...
        void DrawComputer(SplineComputer comp)
        {
            DrawSplineComputer(comp, editorCamera, showThickness, thicknessAutoRotate);
        }
        ...
        public static void DrawSpline(Spline spline, Color color, double from = 0.0, double to = 1.0, bool drawThickness = false, bool thicknessAutoRotate = false)
        {
            ...
            if (drawThickness)
            {
                ...
                while (true)
                {
                    ...
                    SplineResult toResult = spline.Evaluate(percent);
            ...
            else
            {
                ...
                {
                    percent = DMath.Move(percent, to, add);
                    Vector3 toPoint = spline.EvaluatePosition(percent);
                    Handles.DrawLine(fromPoint, toPoint);
                    if (percent == to) break;
                    fromPoint = toPoint;
                }
        ...
        private void EditPoints()
        {
            ...
            {
                if (change) scaleEditor.Reset(ref points, ref selectedPoints);
                if (scaleEditor.SceneEdit(ref points, ref selectedPoints)) change = true;
            }
            else if (tool == PointTool.Rotate)
            {
                if (change) rotationEditor.Reset(ref points, ref selectedPoints);
            ...
            if (emptyClick)
            {
                if (tool != PointTool.Create && Tools.current == Tool.None)
                {
                    if (mouseLeft && !defaultEditor.isDragging && Vector2.Distance(lastClickPoint, Event.current.mousePosition) >= defaultEditor.minimumRectSize && !orbiting) defaultEditor.StartDrag(lastClickPoint);
                }
            }
            if (orbiting && defaultEditor.isDragging) defaultEditor.CancelDrag();
            ...
            if (mouseLeftUp)
            {
                ...
                if (tool == PointTool.Rotate || tool == PointTool.Scale) refreshEditors = true;
            }
        }
       ...
       public void ToggleRotateTool()
        ...
        void GetInput()
        {
            ...
            controlDown = altDown = controlUp = altUp = false;
            switch (e.GetTypeForControl(controlID))
            {
                case EventType.MouseDown:
                    if (e.button == 0)
                    {
                        mouseLeftDown = true;
                        mouseLeft = true;
                        emptyClick = !toolbar.mouseHovers;
                ...
                case EventType.KeyDown:
                    if (e.keyCode == KeyCode.LeftControl || e.keyCode == KeyCode.RightControl || e.keyCode == KeyCode.LeftCommand || e.keyCode == KeyCode.RightCommand) controlDown = control = true;
                    if (e.keyCode == KeyCode.LeftAlt || e.keyCode == KeyCode.RightAlt)
                    {
                        if(!defaultEditor.isDragging) altDown = alt = true;
                    }
                    break;
            ...
            if (!orbiting)
            {
                if (alt && mouseLeft && Event.current.mousePosition != lastClickPoint)
                {
                    orbiting = true;
                    emptyClick = false;
                }
            ...
            if (!mouseRight && !mouseLeft)
            {
                ...
                if (e.commandName == "FrameSelected" && Tools.current == Tool.None)
                {
                    if (points.Length > 0)
                    {
                        e.commandName = "";
